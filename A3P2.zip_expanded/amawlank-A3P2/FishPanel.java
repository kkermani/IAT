...
//	private int foodEaten=0;
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (int p=0;p<predatorList.size();p++) {
			...
			currPredator.checkCollision(this.getSize()); 
			currPredator.move();
			...
			for (int k=0;k<fishList.size();k++) {
				...
				currFish.checkCollision(this.getSize()); 
				currFish.move();
				...
				if(scale>0.4)							
					currFish.energyDecreaseWithSize();	
				currFish.energyDecrease();
				

				if(currFish.inViewOfPredator(currPredator)) {	
